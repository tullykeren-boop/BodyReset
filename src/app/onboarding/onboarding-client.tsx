...
type BodyAreaOption = { slug: string; name: string };
...
export function OnboardingClient({ bodyAreas }: { bodyAreas: BodyAreaOption[] }) {
  const [step, setStep] = useState(0);
  const [answers, setAnswers] = useState<Answers>({ workdayType: null, areas: [], time: null, goal: null });
  ...
  const toggleArea = (slug: string) =>
    setAnswers((a) => ({
      ...a,
      areas: a.areas.includes(slug) ? a.areas.filter((k) => k !== slug) : [...a.areas, slug],
    }));
    ...
    <div className="flex min-h-full flex-col px-6 pb-10 pt-8">
      {step > 0 && step < 5 && (
        <div className="mb-8 flex items-center gap-3">
          <button onClick={back} className="text-ink-soft">
            <ArrowLeft size={18} />
          </button>
      ...
      {step === 0 && (
        <div className="flex grow flex-col items-center justify-center text-center">
          <BreathRing size={130} />
          <h1 className="font-display mt-8 text-3xl leading-tight">Welcome to ReSet</h1>
          <p className="mt-3 max-w-xs text-ink-soft">
            Five quick questions and we&apos;ll build a recovery plan around your body and your day.
          </p>
      ...
      {step === 1 && (
        <div className="flex grow flex-col">
          ...
          <p className="mt-2 text-sm text-ink-soft">This helps us time your sessions around your real schedule.</p>
            ...
            {WORKDAY_TYPES.map((w) => (
              <button
                ...
                className={`w-full rounded-2xl border p-4 text-left transition-colors ${
                  answers.workdayType === w.key ? "border-moss bg-card shadow-sm" : "border-mist bg-white/50"
                }`}
              >
      ...
      {step === 2 && (
        <div className="flex grow flex-col">
          <h2 className="font-display text-2xl leading-tight">Where do you usually feel discomfort?</h2>
          <p className="mt-2 text-sm text-ink-soft">Tap all the areas that apply.</p>
          <div className="mt-4 flex justify-center">
            <BodyMap selected={answers.areas} onToggle={toggleArea} size={130} />
          </div>
      ...
      {step === 3 && (
        <div className="flex grow flex-col">
          ...
          <p className="mt-2 text-sm text-ink-soft">Per session — you can always change this later.</p>
          ...
          <div className="grow" />
          <button onClick={next} disabled={!answers.time} className={primaryButtonClasses("mt-8 w-full")}>
            Continue
          </button>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="flex grow flex-col">
          <h2 className="font-display text-2xl leading-tight">What&apos;s your goal?</h2>
          <p className="mt-2 text-sm text-ink-soft">We&apos;ll shape your plan around this.</p>
          <div className="mt-8 space-y-3">
            {GOALS.map((g) => (
              <button
                key={g.key}
          ...
          <div className="grow" />
          <button onClick={next} disabled={!answers.goal} className={primaryButtonClasses("mt-8 w-full")}>
            Build my plan
          </button>
        </div>
      )}

      {step === 5 && answers.workdayType && answers.time && answers.goal && (
        <PlanGenerating
          answers={{
            workdayType: answers.workdayType,
            painAreaSlugs: answers.areas,
